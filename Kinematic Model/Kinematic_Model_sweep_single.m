...
sweep = 0.10:0.01:0.25;

H = zeros(length(sweep),1);
D = zeros(length(sweep),2);
...
for delta = sweep
    ...
    density = 1900; % kg/m^3
%     delta = 0.25; % deviation percentage
    ...
    H(i) = getdatasamples(h_max,len);
    D(i) = getdatasamples(distance,len);
end
